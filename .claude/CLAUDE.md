...
**Collapsible sections** — `CollapsibleHeadingExtension` (`src/components/editor/extensions/CollapsibleHeadingExtension.ts`) overrides StarterKit's `heading` node. It uses a **DOM-based NodeView** (not React) — React NodeViews use `flushSync` internally, which React 18 forbids inside passive effects where `editor.commands.setContent()` runs; DOM NodeViews avoid this entirely. The NodeView renders each heading element with a collapse chevron button; chevron rotation is applied via `svg.style.transform` (the only permitted inline style for a DOM NodeView, since Tailwind classes can't express dynamic rotation). A ProseMirror plugin rebuilds `DecorationSet` on `tr.docChanged` or `tr.getMeta('rebuildDecorations')`, applying `pm-section-hidden` (`display: none`) to every top-level block between a collapsed heading and the next equal-or-higher-level heading. Collapsed state lives in `useUiStore.collapsedSections: Record<string, string[]>` (noteId → section keys), persisted to localStorage under `m3m-ui`. Section keys are `{level}:{headingText}:{occurrenceIndex}`. On note open, `Editor.tsx` initializes `extension.storage.collapsedSections` from the store **before** `setContent` runs (the two `useEffect` hooks share the same dependency and execute in declaration order), so the plugin's first `tr.docChanged` decoration pass already has the persisted state. `@media print` resets `.pm-section-hidden` to `display: block` so PDFs always include full content.
