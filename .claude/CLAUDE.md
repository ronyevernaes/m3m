...
**Collapsible sections** — `CollapsibleHeadingExtension` (`src/components/editor/extensions/CollapsibleHeadingExtension.ts`) overrides StarterKit's `heading` node and is **fully self-contained**: it implements `addCommands()` (`setHeading` / `toggleHeading`), `addKeyboardShortcuts()` (`Mod-Alt-1` through `Mod-Alt-6`), and `addInputRules()` (`##` → heading input rules via `textblockTypeInputRule`) so that all heading behaviour that StarterKit previously provided is preserved after the override. It uses a **DOM-based NodeView** (not React) — React NodeViews use `flushSync` internally, which React 18 forbids inside passive effects where `editor.commands.setContent()` runs; DOM NodeViews avoid this entirely. The NodeView renders each heading with an **always-visible** `heading-chevron` button positioned absolutely in the left padding via Tailwind (`absolute -left-6 top-1/2 -translate-y-1/2`); the heading `dom` gets `className = 'relative'` to establish the positioning context. Chevron rotation is **CSS-driven**: the plugin adds `Decoration.node` with `{ class: 'heading-is-collapsed' }` on the heading node itself when it is collapsed — ProseMirror merges this class onto the NodeView's outer DOM element — and `.ProseMirror .heading-is-collapsed > .heading-chevron svg { transform: rotate(-90deg) }` in `src/index.css` handles the visual change. The same plugin pass applies `pm-section-hidden` (`display: none`) to every top-level block between a collapsed heading and the next equal-or-higher-level heading. Collapsed state lives in `useUiStore.collapsedSections: Record<string, string[]>` (noteId → section keys), persisted to localStorage under `m3m-ui`. The noteId key is `frontmatter.id ?? note path` — the path fallback ensures notes without a frontmatter ID never share state with each other. Section keys are the **ordinal position** of the heading among all headings in the document (`'0'`, `'1'`, `'2'`, …) — ordinal keys are stable through heading renames and never clash on matching text. On note open, `Editor.tsx` initializes `extension.storage.collapsedSections` from the store **before** `setContent` runs (the two `useEffect` hooks share the same dependency and execute in declaration order). `@media print` hides `.heading-chevron` and resets `.pm-section-hidden` to `display: block` so PDFs always include full content.

**Outline tab** — `OutlineTab` (`src/components/context-panel/OutlineTab.tsx`) is the "Outline" tab in the context panel. It reads `currentNote.body` (markdown string) from the vault store, parses headings via a `useMemo` using a simple regex (`^(#{1,6})\s+(.+)$`) and assigns each heading an ordinal section key (`String(index)`) that matches the keys produced by `findSections()` in `CollapsibleHeadingExtension.ts`. Navigation is driven by `scrollToHeading: ((key: string) => void) | null` in `useUiStore` (non-persisted — excluded from `partialize`). `Editor.tsx` registers this function via a `useEffect([editor])`: it calls `findSections(editor.state.doc)` to resolve the position, dispatches a `TextSelection.near()` transaction to move the cursor, then calls `element.scrollIntoView({ behavior: 'smooth', block: 'start' })` on the heading's DOM node (obtained via `editor.view.nodeDOM(headingPos)`). The OutlineTab calls `useUiStore.getState().scrollToHeading?.(sectionKey)` on click — the optional chain is a no-op when the editor has not yet mounted. Heading indentation uses standard Tailwind `pl-N` classes (h1=`pl-2`, h2=`pl-4`, … h6=`pl-12`).

**Markdown paste** — `MarkdownPasteExtension` (`src/components/editor/extensions/MarkdownPasteExtension.ts`) intercepts plain-text paste events via a ProseMirror plugin. If `text/html` is present in the clipboard (e.g., copying from a browser or rich-text editor), the extension returns `false` and lets TipTap's default HTML paste handler run. If only `text/plain` is available, the text is parsed with `markdownToTipTap()`, converted to a ProseMirror `Slice` via `schema.nodeFromJSON()`, and inserted with `replaceSelection`. When the parsed result is a single paragraph **and** the cursor is inside a text block (`$from.parent.isTextblock`), the slice uses `openStart: 1, openEnd: 1` so inline content (e.g. a pasted URL inside `[Label](`) merges inline rather than creating a new paragraph block. Multi-block pastes (headings, code blocks, lists) always use `openStart: 0, openEnd: 0`.
