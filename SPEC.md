...
- ✅ Context panel — multi-tab inspector (Details, Links, Insights, Agent); resizable; Links tab (`LinksAndBacklinksTab`, `src/components/context-panel/LinksAndBacklinksTab.tsx`) unifies outgoing links and backlinks into two collapsible accordion sub-sections ("Outgoing", "Backlinks") — each section only rendered when it has items; backlinks derived from `note_links` table via `useBacklinks`, live-updated on watcher events; persisted `'backlinks'` tab value migrates to `'links'` transparently; Details tab shows word count, created/modified dates, tags, and custom properties — note `id` is intentionally not displayed (internal identifier)
